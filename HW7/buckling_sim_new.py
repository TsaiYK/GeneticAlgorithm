...
for i in range(nS):
	...
	for j in type_bc:
		...
		p = mdb.models['Model-1'].parts['Force']
		f = p.faces
		pickedFaces = f.findAt(((0.0, 4.0, -2.0), ), ((0.0, 8.0, 2.0), ))
		v2, e1, d2 = p.vertices, p.edges, p.datums
		p.PartitionFaceByShortestPath(faces=pickedFaces, point1=p.InterestingPoint(
			edge=e1.findAt(coordinates=(0.0, 0.0, 3)), rule=MIDDLE), 
			point2=p.InterestingPoint(edge=e1.findAt(coordinates=(0.0, w_domain, 3)), 
			rule=MIDDLE))
		...
		session.viewports['Viewport: 1'].view.setValues(session.views['Iso'])
		# session.viewports['Viewport: 1'].view.setValues(session.views['Iso'])
		# session.viewports['Viewport: 1'].view.setValues(nearPlane=157.062, 
			# farPlane=244.94, width=53.3686, height=25.762, viewOffsetX=-11.4874, 
			# viewOffsetY=-4.63185)
		# a = mdb.models['Model-1'].rootAssembly
		# f1 = a.instances['Skin-1'].faces
		# faces1 = f1.findAt(((-4.0, 0.0, 30.0), ))
		# leaf = dgm.LeafFromGeometry(faceSeq=faces1)
		# session.viewports['Viewport: 1'].assemblyDisplay.displayGroup.remove(leaf=leaf)
		# session.viewports['Viewport: 1'].view.setValues(nearPlane=150.792, 
			# farPlane=251.209, width=93.7527, height=45.2562, viewOffsetX=-0.841036, 
			# viewOffsetY=2.60836)
		# session.viewports['Viewport: 1'].view.setValues(nearPlane=141.124, 
			# farPlane=263.542, width=87.7416, height=42.3545, cameraPosition=(68.7378, 
			# -69.3967, 219.803), cameraUpVector=(-0.0276127, 0.999575, 0.00932738), 
			# cameraTarget=(-9.0265, -2.90176, 46.7928), viewOffsetX=-0.787111, 
			# viewOffsetY=2.44112)
		# a = mdb.models['Model-1'].rootAssembly
		# s1 = a.instances['Skin-1'].faces
		# side2Faces1 = s1.findAt(((-4.0, 0.0, 30.0), ))
		# region1=a.Surface(side2Faces=side2Faces1, name='m_Surf-3')
		# a = mdb.models['Model-1'].rootAssembly
		# s1 = a.instances['Stiff-1'].faces
		# side1Faces1 = s1.findAt(((-3.333333, 0.0, 30.0), ))
		...
		side2Faces1 = s1.findAt(((-25.0/n_stiff*2, -5.0, 0.0), ), ((-10.0, -5.0, 0.0), ), ((-5.0, 
			5.0, 0.0), ), ((-20.0/n_stiff*2, 5.0, 0.0), ))
		...
		faces1 = f1.findAt(((-25.0/n_stiff*2, -5.0, 0.0), ), ((-10.0, -5.0, 0.0), ), ((-5.0, 5.0, 
			0.0), ), ((-20.0/n_stiff*2, 5.0, 0.0), ))
		leaf = dgm.LeafFromGeometry(faceSeq=faces1)
		session.viewports['Viewport: 1'].assemblyDisplay.displayGroup.remove(leaf=leaf)
		...
		side1Edges2 = s2.findAt(((-w_domain/2, 3.75, 0.0), ))
