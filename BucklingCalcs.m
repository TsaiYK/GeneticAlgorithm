function [sigma_cr_stiff, sigma_cr_skin] = BucklingCalcs(x)
...
%Loop for 
if L/rho > L_rho_EJ 
    sigma_cr_stiff = c*(pi^2*E)/(L/rho)^2;
else 
    sigma_cr_stiff = sigma_y*(1-((sigma_y*(L/rho)^2))/(4*c*pi^2*E));
    
end

%%Skin Buckling
sigma_cr_skin = pi^2*kc*E/12/(1-v^2)*(t_skin/60)^2;
