"""
Description:
This script solves the ABAQUS portion of HW8
...
filename = 'PostData_HW8_buckling_eigenVal.txt'
filename_time = 'PostData_HW8_time.txt'
...
#Change number of laminae to an int 
n_lam = int(n_lam)

## Number of designs in DOE
nS = 1
...
for i in range(nS):
    for j in type_bc:        
        ...
        #Composite AS4/3501
        mdb.models['Model-1'].Material(name='AS4/3501')
        mdb.models['Model-1'].materials['AS4/3501'].Elastic(type=LAMINA, table=((
        20600000.0, 1493889.0, 0.27, 1044000.0, 1044000.0, 1044000.0), ))
        mdb.models['Model-1'].materials['AS4/3501'].Density(table=((0.05708112, ), ))
        ...
        mdb.models['Model-1'].HomogeneousShellSection(name='skin', preIntegrate=OFF, 
            material='Aluminum', thicknessType=UNIFORM, thickness=0.001, 
        ...
        p = mdb.models['Model-1'].parts['Skin']
        f = p.faces
        faces = f.findAt(((30.0, 1.333333, 0.0), ))
        region1 = regionToolset.Region(faces=faces)
        compositeLayup = mdb.models['Model-1'].parts['Skin'].CompositeLayup(
        ...
        #angle of each ply set to 0 each time
        theta_lam = 0.0
        # Loop to create as many plys as are in n_lam
        for k in range(n_lam):
            nameply = 'Ply-'+str(k)
            compositeLayup.CompositePly(suppressed=False, plyName=nameply, region=region1, 
            material='AS4/3501', thicknessType=SPECIFY_THICKNESS, thickness=0.005, 
            orientationType=SPECIFY_ORIENT, orientationValue=theta_lam, 
            additionalRotationType=ROTATION_NONE, additionalRotationField='', 
            axis=AXIS_3, angle=0.0, numIntPoints=3)
            print(theta_lam)
            theta_lam = theta_lam + 90.0/(n_lam-1)
        ...
        side2Faces1 = s1.findAt(((-w_domain/2, 0.0, 30.0), ))
        region1=a.Surface(side2Faces=side2Faces1, name='m_Surf-1')
        a = mdb.models['Model-1'].rootAssembly
        s1 = a.instances['Stiff-1'].faces
        side1Faces1 = s1.findAt(((-w_domain/2, 0.0, 30.0), ))
        ...
        edges2 = e2.findAt(((-w_domain/2, h_stiff/2, 90.0), ))
